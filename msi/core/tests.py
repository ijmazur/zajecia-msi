...
class Ambulance(APITestCase):
    ...
    #Test_my_calls_option_in_viewset
    def test_ambulance_view_set_my_calls(self):
        data = {
            "street": "Leszka",
            "number": 2,
            "city": "Lublin",
        ...
        response = self.client.post(self.url+'ambulances/',data,format='json')
        self.assertEqual(response.status_code,201)
        created_ambulance_response = self.client.get(self.url+'ambulances/?option=my-calls', format='json')
        self.assertEqual(created_ambulance_response.status_code,400)
